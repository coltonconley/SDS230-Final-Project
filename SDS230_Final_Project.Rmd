...
plot(aggcompfin$`Composite Returns` ~ aggcompfin$Year, main = "Scatterplot of Year versus Average Yearly Returns 1926-1979", pch = 24, col = factor(aggcompfin$CompInd), xlab = "Year", ylab = "Average Yearly Returns")
legend("bottomleft", col= factor(aggcompfin$CompInd), legend= levels(aggcompfin$CompInd), pch=16)
# lines() - How do we add in lines
```
Overall, returns seem relatively flat throughout the entire period. Given the number of composite industries, it is difficult to see any single trend in the graph. 
...
```{r}
barplot(aggcompfin$`Composite BE/ME Ratio`[aggcompfin$CompInd == "TMT"], main = "Barplot of TMT BE/ME Ratio from 1926-1979", col = "green", ylab = "Composite BE/ME for TMT")
barplot(aggcompfin$`Composite Size`[aggcompfin$CompInd == "TMT"], main = "Barplot of TMT Size from 1926-1979", col = "green", ylab = "Composite Size for TMT")
barplot(aggcompfin$`Composite Returns`[aggcompfin$CompInd == "TMT"], main = "Barplot of TMT Composite Returns from 1926-1979", col = "green", ylab = "Composite Average Yearly Returns for TMT")
```
[ANALYSIS ON TMT GRAPHS]

Now, we examine the same graphs for the Commodities industry. 
```{r}
barplot(aggcompfin$`Composite BE/ME Ratio`[aggcompfin$CompInd == "Commodities"], main = "Barplot of Commodities BE/ME Ratio from 1926-1979", col = "green", ylab = "Composite BE/ME for Commodities")
barplot(aggcompfin$`Composite Size`[aggcompfin$CompInd == "Commodities"], main = "Barplot of Commodities Size from 1926-1979", col = "green", ylab = "Composite Size for Commodities")
barplot(aggcompfin$`Composite Returns`[aggcompfin$CompInd == "Commodities"], main = "Barplot of Commodities Composite Returns from 1926-1979", col = "green", ylab = "Composite Average Yearly Returns for Commodities")
```
...
Because our data does not include any two-level categorical groups, we find it more useful to examine the correlations between our different continous composite variables which we made graphs for above.
...
Now, let's look on non-linearity, correlation, and histograms all at once for our variables of interest. 
```{r}
chart.Correlation(aggcompfin[,c(9, 11:14)], histogram=TRUE, pch=19)
```

[Create bootstrap for the correlation for the strongest correlation? - ASK COLIN ABOUT THIS TOMORROW]

###Permutation Test 

[MAX WILL TRY TO GET TO TOMORROW]

###Multiple Regressions
[COLTON ]
###ANOVA, Logit, Multinomial, or Webscraping 
ANOVA HERE instead of other methods 
